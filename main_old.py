...
class LowLatencyChatbot:
    ...
    async def chat(self, user_id: str, message: str, session_id: Optional[str] = None) -> ChatResponse:
        ...
        try:
            ...
            logger.info(f"Message: {result['messages']}...")
            ...
            if messages:
                for message in reversed(messages):
                    if isinstance(message, AIMessage):
                        ...
                        break
            final_response = final_response if final_response else "I'm sorry, I couldn't generate a response."
            # final_response = result.get("last_response", "I'm sorry, I couldn't generate a response.")
...
async def cli_chat():
    ...
    user_id = input("Enter user ID: ") or "test_user"
    session_id = str(uuid4())

    print(f"Chat session started (Session ID: {session_id})")
